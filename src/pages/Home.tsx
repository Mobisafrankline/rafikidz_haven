...
function StatsBar() {
  const stats = [
    { n: '50+', label: 'Kids in Care', sub: 'and a growing waiting list' },
  ...
  return (
    <div className="relative z-20 -mt-20 px-6 max-w-7xl mx-auto">
      <div className="glass-dark bg-[#0F3840]/80 rounded-3xl p-8 md:p-12 shadow-2xl border border-white/10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4 divide-x divide-white/10">
          {stats.map((s, i) => (
            <div key={s.label} className={`text-center ${i === 0 || i === 2 ? 'border-none md:border-solid' : 'border-none'}`}>
              ...
              <div className="text-[#F5B429] font-bold text-sm tracking-wide mt-2 uppercase">{s.label}</div>
...
function MissionStrip() {
  return (
    <section className="py-32 bg-white relative overflow-hidden">
      <div className="absolute top-0 right-0 w-1/3 h-full bg-[#FAF6F0] rounded-l-full opacity-50 blur-3xl -z-10 transform translate-x-1/2"></div>
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          <div>
            <div className="inline-block bg-[#2B7E8C]/10 text-[#2B7E8C] px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6">Our Mission</div>
            <h2 className="text-4xl md:text-6xl font-extrabold text-[#1A2E33] mb-8 leading-[1.1]">
            ...
            <Link to="/about" className="group inline-flex items-center gap-3 text-[#2B7E8C] font-bold text-lg hover:text-[#1D5A66] transition-colors">
              ...
              <span className="group-hover:translate-x-1 transition-transform bg-[#2B7E8C]/10 p-2 rounded-full">→</span>
            </Link>
          </div>
          <div className="relative group">
            <div className="rounded-[2.5rem] overflow-hidden aspect-[4/5] shadow-2xl shadow-[#2B7E8C]/20 relative z-10">
              <img
            ...
            <div className="absolute -bottom-8 -left-8 bg-[#1D5A66] text-white rounded-3xl p-8 shadow-2xl max-w-[320px] z-20 group-hover:-translate-y-2 transition-transform duration-500">
            ...
            <div className="absolute -inset-4 border-2 border-[#2B7E8C]/20 rounded-[3rem] -z-0 translate-x-4 translate-y-4"></div>
          </div>
        </div>
      </div>
    </section>
...
function StoriesPreview() {
  ...
  return (
    <section className="py-32 bg-[#0F3840] relative">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            <span className="text-[#F5B429] text-xs font-bold uppercase tracking-widest mb-4 block">Their Stories</span>
            <h2 className="text-4xl md:text-6xl font-extrabold text-white leading-tight">50+ Kids,<br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-[#F5B429] to-white">Limitless Futures</span></h2>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {stories.map(s => (
            <div key={s.name} className="group relative rounded-[2rem] overflow-hidden aspect-[4/5] bg-[#1D5A66]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#0F3840] via-[#0F3840]/40 to-transparent opacity-90 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="absolute inset-0 p-8 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                <span className="inline-block bg-[#F5B429] text-[#0F3840] text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-4 w-max">{s.age}</span>
    ...
    <section className="py-32 bg-[#FAF6F0]">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <span className="inline-block bg-[#2B7E8C]/10 text-[#2B7E8C] px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6">What We Provide</span>
          <h2 className="text-4xl md:text-5xl font-extrabold text-[#1A2E33] mb-6">Holistic Care, Root to Branch</h2>
          <p className="text-[#5A7880] text-lg font-light">We believe in treating every aspect of a child's life to ensure they grow up healthy, educated, and ready for the world.</p>
        </div>
...
function UrgentNeeds() {
  ...
  return (
    <section className="py-32 bg-[#1D5A66] relative overflow-hidden">
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#1D5A66] to-[#0F3840]"></div>
      </div>
          ...
          {needs.map(n => (
            <div key={n.label} className="flex items-center justify-between glass hover:bg-white/20 rounded-2xl px-6 py-5 transition-all hover:scale-[1.02] cursor-default">
              <div className="flex items-center gap-4">
                <span className={`w-3 h-3 rounded-full shrink-0 shadow-lg ${n.hot ? 'bg-[#F5B429] shadow-[#F5B429]/50 animate-pulse' : 'bg-white/30'}`} />
                <span className="text-white font-medium">{n.label}</span>
              </div>
              <span className="text-[#F5B429] font-bold shrink-0 ml-4 bg-[#0F3840]/50 px-3 py-1 rounded-full text-sm">{n.amount}</span>
            </div>
          ))}
        </div>
        <div className="text-center">
          <Link to="/donate" className="group inline-flex items-center gap-3 bg-[#F5B429] hover:bg-white text-[#0F3840] px-10 py-5 rounded-full font-bold text-lg transition-all shadow-[0_0_40px_rgba(245,180,41,0.3)] hover:shadow-[0_0_60px_rgba(255,255,255,0.5)] hover:scale-105">
...
function DonateCallout() {
  return (
    <section className="py-0 relative -mt-16 z-20">
      <div className="max-w-7xl mx-auto px-6">
        <div className="bg-gradient-to-br from-[#2B7E8C] to-[#1D5A66] rounded-[3rem] overflow-hidden shadow-2xl p-10 md:p-16 lg:p-20 relative">
  ...
  return (
    <section className="py-32 bg-white pt-48 -mt-24" id="news-preview">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            <span className="text-[#2B7E8C] text-xs font-bold uppercase tracking-widest mb-4 block">Latest Updates</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-[#1A2E33]">News from the Home</h2>
          </div>
          <Link to="/news" className="group inline-flex items-center gap-2 text-[#2B7E8C] font-bold hover:text-[#1D5A66] transition-colors">
            All news <span className="group-hover:translate-x-1 transition-transform">→</span>
          </Link>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {articles.map(a => (
            <Link key={a.title} to="/news" className="group bg-[#FAF6F0] rounded-3xl p-8 h-full border border-transparent hover:border-[#2B7E8C]/20 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 flex flex-col">
              <div className="flex items-center gap-3 mb-6">
                <span className="bg-white text-[#2B7E8C] text-xs font-bold px-3 py-1.5 rounded-full shadow-sm">{a.tag}</span>
                <span className="text-[#5A7880] text-sm font-medium">{a.date}</span>
              </div>
              <h3 className="font-extrabold text-[#1A2E33] text-xl mb-4 group-hover:text-[#2B7E8C] transition-colors leading-snug">{a.title}</h3>
              <p className="text-[#5A7880] text-sm leading-relaxed font-light flex-grow">{a.excerpt}</p>
              <div className="mt-6 text-[#2B7E8C] font-bold text-sm flex items-center gap-2">
